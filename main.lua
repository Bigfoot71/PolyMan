...
local polygons = {
    ...
    {pm.create.donut(gw-92,92,64,64,32,32,8), randomColor()}    -- donut
}
...
function love.update(dt)
    ...
    if editor then
        ...
        if not click then
            ...
            if love.mouse.isDown(1) then
                ...
                for _, polygon in ipairs(polygons) do
                    local inside, nx, ny, dist = pm.collisions.polyInPolyConvex(temp_rand_poly, polygon[1], true)
            ...
            elseif love.mouse.isDown(2) then
                ...
                for i, polygon in ipairs(polygons) do
                    if pm.collisions.polyInPolyFast(temp_rand_poly, polygon[1], true) then
        ...
        for _, polygon in ipairs(polygons) do
            local inside, nx, ny, dist = pm.collisions.polyInPolyConvex(triangle, polygon[1])
...
function love.draw()
    ...
    str = "Press C to apply convex hull to all polygons"
    strw = font:getWidth(str)
    lg.print(str, gw-strw, 0)
